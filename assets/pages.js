...
function photoChip(ps){
  ...
  return `<span class="ev-chip ev-opt">📷 Optional${ps.max?' · max '+ps.max:''}</span>`;
}
...
function renderChecklist(){
  ...
  if(!State.chk) State.chk={session:'Opening',dept:'ALL',state:{}};
  const s=State.chk;
  setCrumb('✅','Store Operation Checklist',`${isAdmin()?'All stores':State.branch} · ${s.session}`);
  const chips=['ALL',...C.depts].map(d=>`<button class="dept-chip ${d===s.dept?'active':''}" onclick="ckDept('${d}')">${d==='ALL'?'All':esc(d)}</button>`).join('');
  ...
  $('#content').innerHTML=`
   <div class="page-head"><div class="ph-ic">✅</div>
     <div><h2>Store Operation Checklist</h2><p>Tick what is completed; attach a photo where required. Leave unticked if not done.</p></div></div>
   <div class="ck-sessionbar card">
     ...
     <div class="filter"><label>Responsible</label><select>${staff.map(n=>`<option>${esc(n)}</option>`).join('')}</select></div>
...
function ckList(){
  const s=State.chk;
  return DB.checklist.items.map((it,i)=>({i,dept:it[0],area:it[1],task:it[2],when:it[3],photo:photoSpec(it[4])}))
    .filter(r=>(s.dept==='ALL'||r.dept===s.dept) && (s.session==='Opening'?(r.when==='O'||r.when==='A'):(r.when==='C'||r.when==='A')));
}
function ckDraw(){
    ...
    Object.entries(areas).forEach(([area,items])=>{
      ...
      items.forEach(r=>{ const st=State.chk.state[r.i]||{}; const done=st.done;
        ...
        if(r.photo){
          ...
          if(!r.photo.max||have<r.photo.max) slots+=`<label class="ck-slot"><input type="file" accept="image/*" capture="environment" onchange="ckPhoto(this,${r.i})"><span class="ck-slot-empty">📷<small>Photo</small></span></label>`;
          photoHtml=`<div class="ck-photos"><div class="ck-photos-h">${photoChip(r.photo)} <span class="ck-pc ${have>=need?'ok':''}">${have}/${need}</span></div><div class="ck-slots">${slots}</div></div>`;
        }
...
function ckProgress(){
  const rows=ckList(); let done=0,total=rows.length,preq=0,pdone=0;
  rows.forEach(r=>{const st=State.chk.state[r.i]||{}; if(st.done)done++; if(r.photo&&r.photo.req){preq++; if((st.photos||[]).length>=r.photo.min)pdone++;}});
...
function ckTick(i){
  ...
  const ps=photoSpec(DB.checklist.items[i][4]);
  if(!st.done && ps && ps.req && (st.photos||[]).length<ps.min){
    toast(`📷 Attach ${ps.min} photo(s) before marking this done`);
...
function ckPhoto(input,i){const f=input.files&&input.files[0];if(!f)return;const url=URL.createObjectURL(f);const st=State.chk.state[i]=State.chk.state[i]||{};st.photos=st.photos||[];st.photos.push(url);ckDraw();toast('📷 Photo added');}
function ckRmPhoto(e,i,url){e.preventDefault();e.stopPropagation();const st=State.chk.state[i];if(st&&st.photos){st.photos=st.photos.filter(u=>u!==url);ckDraw();}}
function ckSession(v){State.chk.session=v;renderChecklist();}
function ckDept(d){State.chk.dept=d;renderChecklist();}
function ckAll(v){ckList().forEach(r=>{const st=State.chk.state[r.i]=State.chk.state[r.i]||{};st.done=v;});ckDraw();}
function chkSubmit(){
  const miss=ckList().filter(r=>r.photo&&r.photo.req&&(State.chk.state[r.i]||{}).done&&(((State.chk.state[r.i]||{}).photos)||[]).length<r.photo.min);
  if(miss.length){ toast(`📷 ${miss.length} completed task(s) still need their required photo`); ckDraw(); return; }
  const done=ckList().filter(r=>(State.chk.state[r.i]||{}).done).length;
  if(!done){ toast('Tick at least one task before submitting'); return; }
  toast(`✓ ${State.chk.session} checklist submitted${isAdmin()?'':' for '+State.branch} · ${done} done`);
}
...
function renderIssue(){
  ...
  $('#content').innerHTML=`
   <div class="page-head"><div class="ph-ic" style="background:#fdeaea">🚩</div><div><h2>Report an Issue</h2><p>Report any operational issue, request or suggestion — reviewed by management only.</p></div>
     <div class="ph-actions"><div class="seg seg-light"><button class="seg-btn active" onclick="issTab('report')">➕ Report</button><button class="seg-btn" onclick="issTab('analytics')">📊 Analytics</button></div></div></div>
...
function issSubmit(){
  ...
  State.iss={cat:'',photo:null,prio:'Normal'};
  toast(`✓ Submitted to ${DB.modules[mod].short} · ${ref}`); buildSidebar(); renderIssue();
}
...
function renderIssueAnalytics(){
  ...
  $('#content').innerHTML=`
    <div class="page-head"><div class="ph-ic" style="background:#fdeaea">🚩</div><div><h2>Report an Issue · Analytics</h2><p>Breakdown by category, with a side-by-side comparison across branches.</p></div>
      <div class="ph-actions"><div class="seg seg-light"><button class="seg-btn" onclick="issTab('report')">➕ Report</button><button class="seg-btn active" onclick="issTab('analytics')">📊 Analytics</button></div></div></div>
