...
Generated by AI Lab (demo) — manager to review before any action.`;
  ...
  function aiLocalParse(text, roster, stores){
    ...
    // smart staff match across ALL stores: full-name substring OR any first/last-name token
    // present as a word; prefer the detected store; best (longest / most tokens) wins.
    const words=new Set(tn.split(/[^a-z0-9]+/).filter(Boolean));
    let staff='', bestScore=0;
    (roster||[]).forEach(r=>{ const full=norm(r.name); if(!full) return;
      const toks=full.split(/[^a-z0-9]+/).filter(t=>t.length>=2);
      let score=0;
      ...
      if(score>0){ if(store && r.store===store) score+=100; if(score>bestScore){ bestScore=score; staff=r.name; } }
    });
  ...
  function asstConfirm(intent, text){
    ...
    const pool=(DB.staff||[]).filter(s=>s.active!==0 && (isSuper()||s.store===State.branch));
    const matches=isAnn?[]:asstResolveStaff(intent.staff,intent.store);
