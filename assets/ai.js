...
Generated by AI Lab (demo) — manager to review before any action.`;
  ...
  function aiAssistant(){
    const examples=[
      ...
      'Gửi cảnh cáo cho <tên> ở Subiaco vì đi trễ',
      'Send the new roster document to <name>',
      'Email <name> about the shift change',
      'Announce the Tết closing dates to all stores',
    ];
  ...
  async function asstParse(text){
    ...
    try{ if(window.mcqAiCommand){ const r=await mcqAiCommand(text, roster, stores); if(r&&r.ok&&r.intent&&r.intent.action) return r.intent; } }catch(e){}
  ...
  function aiLocalParse(text, roster, stores){
    ...
    let store=''; (stores||[]).forEach(s=>{ if(tn.includes(norm(s))) store=s; });
    let staff=''; (roster||[]).forEach(r=>{ const n=norm(r.name); if(n && tn.includes(n) && r.name.length>staff.length) staff=r.name; });
    const all=/all|company|tat ca|toan|every store/.test(tn);
    return {action, staff, store, subject:'', body:'<p>'+esc(text)+'</p>', rule: action==='violation'?text:'',
      severity:'Minor', step:'Verbal Discussion', scope: (action==='announcement'&&all)?'all':'store'};
  }
  ...
  function asstResolveStaff(name){
    const norm=(window.staffNorm||function(s){return String(s||'').toLowerCase();});
    const pool=(DB.staff||[]).filter(s=>s.active!==0 && (isSuper()||s.store===State.branch));
    if(!name) return [];
    const n=norm(name);
    const exact=pool.filter(s=>norm(s.name)===n); if(exact.length) return exact;
    return pool.filter(s=>norm(s.name).includes(n)||n.includes(norm(s.name)));
  }
  function asstConfirm(intent, text){
    const act=intent.action, isAnn=act==='announcement';
    const pool=(DB.staff||[]).filter(s=>s.active!==0 && (isSuper()||s.store===State.branch));
    const matches=isAnn?[]:asstResolveStaff(intent.staff);
    const bestId=matches[0]&&matches[0].id;
    const staffSel=isAnn?'':`<div class="field"><label>Employee</label><select id="asst-staff">${pool.map(s=>`<option value="${esc(s.id)}" ${bestId===s.id?'selected':''}>${esc(s.name)} · ${esc(s.store)}${s.email?'':' · (no email)'}</option>`).join('')||'<option value="">— no staff —</option>'}</select></div>`;
    ...
    const vioFields=act==='violation'?`<div class="grid2">
        <div class="field"><label>Rule / reason</label><input id="asst-rule" value="${esc(intent.rule||intent.subject||'')}"></div>
    ...
    const bodyLabel=act==='violation'?'Description':'Message';
    ...
    const warn=(!isAnn && !matches.length)?`<div class="ai-asst-err">⚠️ I couldn’t match “${esc(intent.staff||'')}”. Please pick the employee below.</div>`:'';
    window.__asstIntent=intent;
    mcqModal(`🤖 Confirm — ${actName}`, `${warn}${storeSel}${staffSel}${vioFields}${subjRow}
      <div class="field"><label>${bodyLabel}</label><textarea id="asst-body" rows="6"></textarea></div>
      <div class="ai-asst-note">Review &amp; edit, then confirm. This will ${act==='email'?'send an email':act==='announcement'?'post an announcement':'deliver to the employee’s inbox'}.</div>
    ...
    const b=document.getElementById('asst-body'); if(b) b.value=intent.body||('<p>'+esc(text)+'</p>');   // seed BEFORE mount so CKEditor picks it up
  ...
  function asstSend(){
    ...
    const sid=document.getElementById('asst-staff')?.value;
    const s=(DB.staff||[]).find(x=>String(x.id)===String(sid));
    if(!s){ toast('Pick an employee'); return; }
    if(act==='violation'){
      const rule=(document.getElementById('asst-rule')?.value||'Policy violation').trim();
      ...
      const rec=window.mcqCreateViolation&&mcqCreateViolation({staff:s.name,store:s.store,ruleTitle:rule,severity:sev,step,description:String(body).replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim()});
      toast(rec?('✓ Violation sent to '+s.name):'Could not create'); if(window.mcqRefreshUnread)mcqRefreshUnread(); mcqModalClose(); return;
    }
    const subj=(document.getElementById('asst-subj')?.value||intent.subject||'Message').trim();
    if(act==='document'){
      if(window.mcqMsgSend) mcqMsgSend({kind:'document', store:s.store, to_staff_id:s.id, subject:subj, body_html:body}).then(r=>{ toast(r&&r.ok?('📄 Document sent to '+s.name):'Could not send'); if(window.mcqRefreshUnread)mcqRefreshUnread(); });
      mcqModalClose(); return;
    }
    if(act==='email'){
      if(!s.email){ toast('⚠️ '+s.name+' has no email on file'); return; }
      if(window.mcqEmail&&mcqEmail.sendHtml) mcqEmail.sendHtml([{email:s.email,name:s.name}], subj, body);
      mcqModalClose(); return;
    }
  }
