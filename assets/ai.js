...
Generated by AI Lab (demo) — manager to review before any action.`;
  ...
  async function asstParse(text){
    ...
    try{ if(window.mcqAiCommand){ const r=await mcqAiCommand(text, roster, stores, rules); if(r&&r.ok&&r.intent&&r.intent.action&&r.intent.action!=='unknown'){
      ...
      if(it.action==='violation' && !it.description){ it.description=aiVioDescription(it.reason||it.rule||text, aiMatchRule(it.rule||it.reason||text)); }
      return it;
    } } }catch(e){}
  ...
  function aiLocalParse(text, roster, stores){
    ...
    // match ALL people named (across every store) — supports sending to several at once
    ...
    // reason = the part after "for/about/vì…"
    const m=text.match(/\b(?:for|about|due to|because of|regarding|vì|về|do)\b\s+(.+)$/i);
    const reason=m?m[1].trim():'';
    const rule=action==='violation'?aiMatchRule(reason||text):null;
    const all=/all|company|tat ca|toan|every store/.test(tn);
    return { action, store:storesHit[0]||'', recipients, staff:(recipients[0]&&recipients[0].name)||'',
